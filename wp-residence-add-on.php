...
switch( $custom_type ) {
	...
	case "estate_property":
		...
		$wpresidence_addon->add_title( 'Property Subunits' );

		$wpresidence_addon->add_field( 'enable_subunits', 'Enable Subunits?', 'radio', array(
			'no' => 'No',
			'yes' => array(
				'Yes',
				$wpresidence_addon->add_field( 'property_subunits_list', 'Subunit IDs', 'text', null, 'Comma delimited list of subunit IDs' )
				)
			)
		);
		...
		function wpresidence_addon_import( $post_id, $data, $import_options, $article ) {
			...
			// Take care of property subunits
			...
			if ( empty( $article['ID'] ) or $wpresidence_addon->can_update_meta( $field, $import_options ) ) {
				...
				if ( $data[ $field ] == 'yes' && !empty( $data[ 'property_subunits_list'] ) ) {
					...
					if ( empty( $article['ID'] ) or $wpresidence_addon->can_update_meta( 'property_subunits_list', $import_options ) ) {

						update_post_meta( $post_id, 'property_subunits_list', explode( ",", $data['property_subunits_list'] ) );

					}

					if ( empty( $article['ID'] ) or $wpresidence_addon->can_update_meta( 'property_subunits_list_manual', $import_options ) ) {

						update_post_meta( $post_id, 'property_subunits_list_manual', $data['property_subunits_list'] );

					}

					if ( empty( $article['ID'] ) or $wpresidence_addon->can_update_meta( 'property_has_subunits', $import_options ) ) {

						update_post_meta( $post_id, 'property_has_subunits', '1' );
					...
					if ( empty( $article['ID'] ) or $wpresidence_addon->can_update_meta( 'property_has_subunits', $import_options ) ) {
						...
						update_post_meta( $post_id, 'property_has_subunits', '' );

					}

					if ( empty( $article['ID'] ) or $wpresidence_addon->can_update_meta( 'property_subunits_list_manual', $import_options ) ) {

						update_post_meta( $post_id, 'property_subunits_list_manual', '' );
...
function property_images( $post_id, $attachment_id, $image_filepath, $import_options ) {
	...
	update_post_meta( $post_id, 'image_to_attach', trim( $current_images, "," ) . "," );
}
